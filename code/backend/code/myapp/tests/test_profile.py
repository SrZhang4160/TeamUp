...
class ProfileTest(TestCase):
    ...
    def test_userinfo(self, enable=False):
        if enable:
            client = Client()
            json_data = {
                        "name": "testname",
            ...
            response_json = post_request(client, "create_student_api", json_data)
            try:
                json_data = {"HTTP_X_TOKEN" : response_json['HTTP_X_TOKEN'] }
                response_json = post_request(client, "userinfo", json_data)
            except:
                print("Error: can not get HTTP_X_TOKEN")
            return response_json
        else:
            pass

    def test_create_login_fill_profile(self, return_json=None, enable=True):
        if enable:
            client = Client()
            json_data = {
                        "email": "admin",
