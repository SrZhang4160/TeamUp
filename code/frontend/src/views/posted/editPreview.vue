    ...
    <div class="preview_content">
      <div class="editpreview_list">
        ...
        >
          ...
          <div>Important</div>
          <div>
            <el-select v-model="item.criteriaPption" @change="change(index)"  placeholder="please change">
              <el-option
    ...
    <div class="back_grouping">
      <div>
        <el-button type="primary" plain @click="confirm">
          Group all students with One click
        </el-button>
      </div>
      <div>
        <el-button type="primary" plain @click="confirm2">
          Group students without groups with One click
        </el-button>
      </div>
    </div>
  </div>
...
<script>
import { editList,preserve,preserve2} from "@/api/preview";
export default {
  data() {
    return {
      value2: 15,
  ...
  methods:{
      ...
      confirm(){
        //  点击保存接口
        preserve({criteriaList:this.userList}).then(res=>{
          if(res.code==1){
      ...
      confirm2(){
        //  点击保存接口
        preserve2({criteriaList:this.userList}).then(res=>{
          if(res.code==1){
            /* const routerJump = this.$router.resolve({
              path: "/preview/preview"
            });
